...
export async function renderPitchCharts(result, options = {}) {
  ...
  async function renderPointCharts(windowIndex) {
    ...
    const sourcePeakBin =
      Array.isArray(debug.seedPeakBins) && Number.isFinite(debug.seedPeakBins[0])
        ? debug.seedPeakBins[0]
        : Number.isFinite(debug.strongestPeakBin)
          ? debug.strongestPeakBin
          : 0;
    ...
    const markerX = Number.isFinite(selectedPeakWeightedBin)
      ? [selectedPeakWeightedBin * result.binSizeHz]
      : [];
  ...
  function updateSelectedHzTitle(windowIndex) {
    const hz = result.pitchHz[windowIndex];
    const selectedText = Number.isFinite(hz) ? `${hz.toFixed(2)} Hz` : "n/a";
