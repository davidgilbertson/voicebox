...
const VibratoChart = forwardRef(function VibratoChart(
  ...
  const vibratoRatePillPct =
    vibratoRatePositionPct === null ? null : clamp(vibratoRatePositionPct, 8, 92);
  const sweetStartPct =
    ((VIBRATO_SWEET_MIN_HZ - VIBRATO_RATE_MIN_HZ) / (VIBRATO_RATE_MAX_HZ - VIBRATO_RATE_MIN_HZ)) *
    100;
  const sweetEndPct =
    ((VIBRATO_SWEET_MAX_HZ - VIBRATO_RATE_MIN_HZ) / (VIBRATO_RATE_MAX_HZ - VIBRATO_RATE_MIN_HZ)) *
    100;
      ...
      <div className="pointer-events-none px-2 pt-1 pb-2">
        <div className="relative">
          ...
          {vibratoRatePositionPct !== null ? (
            <>
              <div
                className="absolute -top-8 -translate-x-1/2 rounded-full border border-slate-300/20 bg-slate-900/85 px-2 py-0.5 text-xs font-medium whitespace-nowrap text-slate-100"
        ...
        <div className="relative mt-1 h-3 text-xs leading-none text-slate-400/65">
          <span className="absolute top-0 left-0">{VIBRATO_RATE_MIN_HZ} Hz</span>
          <span
            className="absolute top-0 -translate-x-1/2 text-slate-300/85"
            style={{ left: `${sweetStartPct}%` }}
          >
            {VIBRATO_SWEET_MIN_HZ} Hz
          </span>
          <span
            className="absolute top-0 -translate-x-1/2 text-slate-300/85"
            style={{ left: `${sweetEndPct}%` }}
          >
          ...
          {intermediateHzLabels.map((hz) => (
            <span
              ...
              className="absolute top-0 -translate-x-1/2 text-slate-300/85"
              style={{
                left: `${((hz - VIBRATO_RATE_MIN_HZ) / (VIBRATO_RATE_MAX_HZ - VIBRATO_RATE_MIN_HZ)) * 100}%`,
              }}
            >
              {hz} Hz
            </span>
          ))}
          <span className="absolute top-0 right-0">{VIBRATO_RATE_MAX_HZ} Hz</span>
        </div>
      </div>
    </>
  );
