export default function StepperControl({
  ...
  return (
      <div className="inline-flex h-10 items-stretch rounded-md bg-slate-800/80 px-1">
        <button
            type="button"
            onClick={onDecrement}
            disabled={decrementDisabled}
            className="inline-flex h-full select-none touch-manipulation items-center justify-center rounded-md px-3 text-lg font-semibold text-slate-100 disabled:opacity-40"
            aria-label={decrementAriaLabel}
        >
          -
        </button>
        <div
            className={`inline-flex items-center justify-center px-2 ${valueClassName}`}
            style={contentStyle}
        >
          {value}
        </div>
