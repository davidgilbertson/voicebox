...
export default function StepperControl({
  ...
  return (
      <div className={`inline-flex items-stretch rounded-md bg-slate-800/80 ${isLarge ? "h-16" : "h-11"}`}>
        <button
            type="button"
            onClick={onDecrement}
            disabled={decrementDisabled}
            className={`inline-flex h-full select-none touch-manipulation items-center justify-center rounded-md text-slate-500 disabled:opacity-40 ${
                isLarge ? "px-4" : "px-3"
            }`}
